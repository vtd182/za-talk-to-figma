...
const serializerDebugEnabled = true;

const serializerLog = (message: string, extra?: Record<string, unknown>) => {
  if (!serializerDebugEnabled) return;
  const prefix = `[za-talk-to-figma:serializer] ${message}`;
  if (extra) {
...
export const serializeNode = async (node: any): Promise<any> => {
  if ("children" in node && node.children.length > 100) {
    serializerLog("serializing large subtree", {
      id: node.id,
      name: node.name,
      type: node.type,
  ...
  if ("children" in node) {
    ...
    if (node.children.length > 100) {
      serializerLog("finished large subtree", {
        id: node.id,
        name: node.name,
        type: node.type,
        childCount: node.children.length,
