...
// loadTextNodeFonts loads every font used by a text node so its properties can
// be mutated. A node can use multiple fonts across character ranges, in which
// case fontName is figma.mixed and each range's font must be loaded.
export const loadTextNodeFonts = async (node: TextNode): Promise<void> => {
