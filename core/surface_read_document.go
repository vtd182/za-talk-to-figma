...
func registerReadDocumentTools(s *server.MCPServer, runtime *Runtime) {
	...
	), makeHandler(runtime, "get_document", nil, nil))
	...
	s.AddTool(mcp.NewTool("get_pages",
		...
		withOptionalSessionTarget(),
	), makeHandler(runtime, "get_pages", nil, nil))
	...
	s.AddTool(mcp.NewTool("get_metadata",
		...
		withOptionalSessionTarget(),
	), makeHandler(runtime, "get_metadata", nil, nil))
	...
	s.AddTool(mcp.NewTool("get_selection",
		...
		withOptionalSessionTarget(),
	), makeHandler(runtime, "get_selection", nil, nil))

	s.AddTool(mcp.NewTool("get_node",
		mcp.WithDescription("Get a single node by ID. By default this attempts full detail, but on very large/deep subtrees it may return partial/truncated data with fallback hints instead of hanging. Node ID must be colon format e.g. '4029:12345', never hyphens."),
		mcp.WithString("nodeId",
			mcp.Required(),
			mcp.Description("Node ID in colon format e.g. '4029:12345'"),
		),
		mcp.WithString("detail",
			mcp.Description("Read verbosity: minimal, summary, compact, or full (default full)"),
		),
		mcp.WithNumber("depth",
			mcp.Description("Maximum traversal depth before returning childCount/truncated fallback"),
		),
		mcp.WithNumber("maxNodes",
			mcp.Description("Traversal node budget before returning partial data"),
		),
		mcp.WithNumber("maxTimeMs",
			mcp.Description("Traversal time budget in milliseconds before returning partial data"),
		),
		mcp.WithBoolean("compactInstances",
			mcp.Description("When true, nested INSTANCE subtrees are compacted instead of fully expanded"),
		),
		withOptionalSessionTarget(),
	), func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		nodeID, _ := req.GetArguments()["nodeId"].(string)
		params := collectReadTraversalParams(req)
		resp, err := runtime.Engine.Execute(ctx, "get_node", []string{nodeID}, params)
		return renderResponse(resp, err)
	})

	s.AddTool(mcp.NewTool("get_nodes_info",
		mcp.WithDescription("Get details for multiple nodes by ID in one round-trip. On heavy subtrees it may compact/trim results instead of timing out."),
		...
		mcp.WithString("detail",
			mcp.Description("Read verbosity: minimal, summary, compact, or full (default compact for multi-node reads)"),
		),
		mcp.WithNumber("depth",
			mcp.Description("Maximum traversal depth before returning childCount/truncated fallback"),
		),
		...
		params := collectReadTraversalParams(req)
		resp, err := runtime.Engine.Execute(ctx, "get_nodes_info", nodeIDs, params)
		return renderResponse(resp, err)
	})

	s.AddTool(mcp.NewTool("get_node_context",
		mcp.WithDescription("Get a depth-limited, fallback-safe view of a single node. Prefer this over get_node when working with large screens or deeply nested instances."),
		mcp.WithString("nodeId",
			mcp.Required(),
			mcp.Description("Node ID in colon format e.g. '4029:12345'"),
		),
		mcp.WithString("detail",
			mcp.Description("Read verbosity: minimal, summary, compact, or full (default compact)"),
		),
		mcp.WithNumber("depth",
			mcp.Description("Maximum traversal depth before returning childCount/truncated fallback"),
		),
	...
	s.AddTool(mcp.NewTool("get_design_context",
		...
		resp, err := runtime.Engine.Execute(ctx, "get_design_context", nil, params)
		return renderResponse(resp, err)
	})
	...
	s.AddTool(mcp.NewTool("search_nodes",
		...
		mcp.WithNumber("maxVisited",
			mcp.Description("Traversal node budget before returning partial results"),
		),
		mcp.WithNumber("maxTimeMs",
			mcp.Description("Traversal time budget in milliseconds before returning partial results"),
		),
		withOptionalSessionTarget(),
	), func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if maxVisited, ok := req.GetArguments()["maxVisited"].(float64); ok && maxVisited > 0 {
			params["maxVisited"] = maxVisited
		}
		...
		resp, err := runtime.Engine.Execute(ctx, "search_nodes", nil, params)
		return renderResponse(resp, err)
	})
	...
	s.AddTool(mcp.NewTool("scan_text_nodes",
		...
		params = injectOptionalSession(params, req)
		resp, err := runtime.Engine.Execute(ctx, "scan_text_nodes", nil, params)
		return renderResponse(resp, err)
	})
	...
	s.AddTool(mcp.NewTool("scan_nodes_by_types",
		...
		params = injectOptionalSession(params, req)
		resp, err := runtime.Engine.Execute(ctx, "scan_nodes_by_types", nil, params)
		return renderResponse(resp, err)
	})
	...
	s.AddTool(mcp.NewTool("get_reactions",
		...
		nodeID, _ := req.GetArguments()["nodeId"].(string)
		resp, err := runtime.Engine.Execute(ctx, "get_reactions", []string{nodeID}, injectOptionalSession(nil, req))
		return renderResponse(resp, err)
	})
	...
	s.AddTool(mcp.NewTool("get_viewport",
		...
		withOptionalSessionTarget(),
	), makeHandler(runtime, "get_viewport", nil, nil))
	...
	s.AddTool(mcp.NewTool("get_fonts",
		...
		params = injectOptionalSession(params, req)
		resp, err := runtime.Engine.Execute(ctx, "get_fonts", nil, params)
		return renderResponse(resp, err)
	})
}

func collectReadTraversalParams(req mcp.CallToolRequest) map[string]interface{} {
	params := map[string]interface{}{}
